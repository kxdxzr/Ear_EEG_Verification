...
class Filtering_BSF:
    ...
    frequency_low = 40
    high_pass = Filtering_HPF(frequency_high,fs,order)
    low_pass = Filtering_LPF(frequency_low,fs,order)
    
    def __init__(self,frequency_high,frequency_low,sampling_rate,order): 
        self.frequency_high = frequency_high
        self.frequency_low = frequency_low
        self.order = order
        self.fs = sampling_rate
        self.high_pass = Filtering_HPF(self.frequency_high,self.fs,self.order)
        self.low_pass = Filtering_LPF(self.frequency_low,self.fs,self.order)
